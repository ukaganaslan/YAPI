...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Demo fallback: any .edu email works
      ...
      if (/\.edu(\.[a-z]{2})?$/i.test(email)) {
        localStorage.setItem('user', JSON.stringify({ name: email.split('@')[0], email, role: 'Researcher' }));
        navigate('/dashboard');
      } else {
        setError('Connection error. Please ensure the mock server is running.');
      }
    } finally {
      setLoading(false);
    }
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center px-4 pt-16">
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-slate-500 text-sm">Sign in to your research account</p>
        </div>
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  placeholder="••••••••"
                  required
                  className="w-full px-4 py-3 pr-12 border border-slate-300 rounded-xl text-slate-900 placeholder-slate-400 focus:border-blue-500 transition-colors text-sm"
                />
                <button
                  type="button"
        ...
        <div className="mt-4 p-4 bg-amber-50 border border-amber-200 rounded-xl text-center">
          <p className="text-amber-800 text-xs font-medium">
            Demo: Use any <code className="bg-amber-100 px-1 py-0.5 rounded text-xs font-mono">@university.edu</code> email with any password
          </p>
        </div>
      </div>
    </div>
